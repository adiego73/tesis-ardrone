...
// informacion en la pantalla.
    std::string line1 = cv::format( "SET-> Pitch: %.3f, Roll: %.3f, Yaw: %.3f, Z: %.3f, Dist: %.1f, D_x: %.1f, D_y: %.1f",
    ...
    std::string line3 = cv::format(
                            "Vel-Y: %.3f, Vel-X: %.3f, Vel-Z: %.3f cm/s",
                            velocity.y, velocity.x, velocity.z );

    std::string line4 = cv::format( "Destino: X: %.1f, Y: %.1f Bateria: %d &&",
                                    env->getNextDestination().x, env->getNextDestination().y,
                                    messageServer->getInt( "robot/battery", 0 ) );

    std::string line5 = cv::format( "POSICION--> X: %.2f, Y: %.2f, Z: %.2f Tierra: %s Estado: %d AutoControl: %s",
    ...
    cv::putText( frame, line1, cv::Point( 10, 10 ), CV_FONT_HERSHEY_SIMPLEX, 0.4, CV_RGB( 255, 0, 0 ) );
    cv::putText( frame, line2, cv::Point( 10, 30 ), CV_FONT_HERSHEY_SIMPLEX, 0.4, CV_RGB( 0, 0, 255 ) );
    cv::putText( frame, line3, cv::Point( 10, 50 ), CV_FONT_HERSHEY_SIMPLEX, 0.4, CV_RGB( 0, 0, 255 ) );
    cv::putText( frame, line4, cv::Point( 10, 70 ), CV_FONT_HERSHEY_SIMPLEX, 0.4, CV_RGB( 0, 0, 0 ) );
    cv::putText( frame, line5, cv::Point( 10, 90 ), CV_FONT_HERSHEY_SIMPLEX, 0.4, CV_RGB( 0, 0, 0 ) );
